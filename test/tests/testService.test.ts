...
describe('Apex Test Suites', async () => {
  ...
  describe('Build Test Payload', async () => {
    ...
    it('should correctly identify and separate Flow tests from Apex tests', async () => {
      ...
      const testService = new TestService(mockConnection);
      // Mock the processTestWithPrefix and processApexTest methods
      const processTestWithPrefixSpy = sandboxStub.spy(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        testService as any,
        'processTestWithPrefix'
      );
      ...
      // Verify that processTestWithPrefix was called for Flow test
      expect(processTestWithPrefixSpy.calledOnce).to.be.true;
      expect(processTestWithPrefixSpy.args[0][0]).to.deep.equal([
        'FlowTesting',
        'TestFlow',
        'TestFlowClass',
        'method1'
    ...
    it('should handle only Flow tests in test payload', async () => {
      ...
      // Verify that only processTestWithPrefix was called
      expect(processTestWithPrefixSpy.calledTwice).to.be.true;
    ...
    it('should handle only Apex tests in test payload', async () => {
      ...
      expect(processTestWithPrefixSpy.notCalled).to.be.true;
    });
  });
